...
def RunUnitAndIntegrationTests(self: FireXTask, uid):
    ...
    unit_tests_xunit, unit_tests_coverage_dat = get_results(
        ut_promise, return_keys=("unit_tests_xunit", "unit_tests_coverage_dat")
    )

    integration_tests_xunits, integration_tests_coverage_dats = get_results(
        it_promise,
        return_keys=("integration_tests_xunits", "integration_tests_coverage_dats"),
    )
...
def RunAllIntegrationTests(
    ...
    for config in discover_tests(integration_tests_dir):
        ...
        integration_tests_coverage_dats.append(
            os.path.join(test_config_output_dir, ".coverage")
        )
        parallel_tasks[f"{test_config_filepath}.{test_config_name}"] = (
            RunIntegrationTests.s(
                uid=uid,
...
def GenerateHtmlCoverage(uid, coverage_dat_file):
    ...
    firex_subprocess.run(
        ["coverage", "html", "--title", "Code Coverage"],
        cwd=uid.logs_dir,
        env=env,
        check=True,
    )
...
def AggregateCoverage(uid, coverage_files):
    ...
    firex_subprocess.run(
        ["coverage", "combine"] + coverage_files, check=True, cwd=uid.logs_dir, env=env
    )
...
def AggregateXunit(uid, xunit_result_files):
    ...
    aggregated_xunit_results = os.path.join(
        uid.logs_dir, "aggregated_xunit_results.xml"
    )
