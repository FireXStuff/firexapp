...
def discover_tests(tests, config_filter="") -> list:
    ...
    for tests_path in tests.split(","):
        if not os.path.exists(tests_path):
            print(
                "Error: --tests must be a directory or a python module containing test configs\n"
                f"{tests_path} is not recognized",
                file=sys.stderr,
            )
            sys.exit(-1)
    ...
    [
        print("Skipping " + config.__class__.__name__, file=sys.stderr)
...
def import_test_configs(path) -> list[FlowTestConfiguration]:
    ...
    if (
        __file__ in path
        or "pycache" in path  # We don't need to look at the cache
        or os.path.basename(path)
        == "data"  # By convention, a "data" directory will contain artifacts for the tests
    ):
        return []
    ...
    if (
        ...
        for _, obj in inspect.getmembers(module, inspect.isclass):
            if (
                FlowTestConfiguration.__name__
                in [cls.__name__ for cls in inspect.getmro(obj)[1:]]
                and not inspect.isabstract(obj)
                and "__metaclass__" not in obj.__dict__
