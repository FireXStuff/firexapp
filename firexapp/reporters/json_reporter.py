...
class FireXRunData:
    ...
    @classmethod
    def _create_from_dict(cls: Type[T], run_dict: dict[str, Any]) -> T:
        field_names = {
        ...
        return cls(_extra_fields=extra_fields, **modelled_fields)
    ...
    @classmethod
    def load_run_json_file(cls: Type[T], json_filepath: str) -> T:
        with open(json_filepath, encoding='utf-8') as f:
            return cls._create_from_dict(json.load(fp=f))

    @classmethod
    def load_initial(cls: Type[T], logs_dir: str) -> T:
        return cls.load_run_json_file(_get_initial_run_json_path(logs_dir))
    ...
    def write_run_completed(
        ...
        if self.completed_timestamp is None:
            self.completed_timestamp = datetime.datetime.now(datetime.timezone.utc)
    ...
    def get_status_and_description(self) -> tuple['FireXRunStatus', str]:
        ...
        if self.revoked:
            ...
            return FireXRunStatus.REVOKED, revoked_description #_revoke_description(self.logs_path)
...
def _get_completed_revoke_details(
    ...
    try:
        ...
        if revoked_details:
            revoked_details.write_revoke_complete()
...
class RevokeDetails:
    ...
    revoke_start_time: datetime.datetime = dataclasses.field(default_factory=datetime.datetime.now)
    ...
    def write_revoke_complete(self):
        if self.revoke_complete_time is None:
            self.revoke_complete_time = datetime.datetime.now()
            self.write()
...
class FireXJsonReportGenerator:
    ...
    def create_completed_run_json(
        ...
        **inputs,
        ...
        report_link = os.path.join(logs_path, FireXJsonReportGenerator.report_link_filename)
