<template>
  <div>
    <div class="header">
      ...
      <div class="header">
        <a href="/" class="logo"><img style='height: 36px;' src="/static/firex_logo.png"></a>
        <div class="uid">{{title}}</div>
        <a class="flame-link" href="help">Help</a>
        <a :href="'http://firex.cisco.com' + logDir" class="flame-link">View Logs</a>
    ...
    <router-view v-on:title="title = $event" :root="root"></router-view>
  </div>
</template>
...
import {parseRecFileContentsToTree} from '../parse_rec.js'
    ...
    root () {
      return fetch(this.logDir + '/flame.rec')
        ...
        .then(function (rec_file_content) {
          return parseRecFileContentsToTree(rec_file_content)
        })
    }
  }
}
