...
export default {
  name: 'XLinks',
  props: {
    uncollapsedIntrinsicDimensionNodesByUuid: { required: true, type: Object },
    nodeLayoutsByUuid: { required: true, type: Object },
  },
  computed: {
    nodesByUuid() {
      return _.mapValues(this.uncollapsedIntrinsicDimensionNodesByUuid,
    ...
    displayLinks() {
      ...
      _.each(this.nodeLayoutsByUuid, (__, cUuid) => {
        ...
        if (!_.isNull(parentId) && _.has(this.nodesByUuid, parentId)) {
          ...
          const d = `M${p.x + p.width / 2} ${p.y + p.height
