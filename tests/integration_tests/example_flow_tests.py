...
class GreetGuestsTest(FlowTestConfiguration):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        ...
        assert (
            self.completed_run.get_result("guests_greeting")
            == "Hello John! Hello Mohammad!"
        )
...
class GreetSpringfieldPowerPlantTest(FlowTestConfiguration):
    ...
    def initial_firex_options(self) -> list:
        return [
            "submit",
            "--chain",
            "greet_springfield_power_plant_employees",
            "--employee_names",
            "Waylon Smithers,Homer Simpson",
            "--celery_concurrency",
            "10",
        ]
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        expected_result = (
            "HELLO EXECUTIVE ASSISTANT WAYLON SMITHERS! HELLO SUPERVISOR HOMER SIMPSON!"
        )
        actual_result = self.completed_run.chain_results()["amplified_greeting"]
        assert actual_result == expected_result, (
            f"Expected '{expected_result}'  \n Received '{actual_result}'"
...
class GreetSpringfieldPowerPlantWithPluginTest(FlowTestConfiguration):
    def initial_firex_options(self) -> list:
        return [
            ...
            "greet_springfield_power_plant_employees",
            "--employee_names",
            "Homer Simpson,Waylon Smithers",
            "--plugins",
            os.path.join(test_data_dir, "plugins", "springfield_monarchy.py"),
            "--celery_concurrency",
            "10",
        ]
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        expected_result = (
            "HELLO CHANCELLOR HOMER SIMPSON! HELLO PRINCE WAYLON SMITHERS!"
        )
        actual_result = self.completed_run.chain_results()["amplified_greeting"]
        assert actual_result == expected_result, (
            f"Expected '{expected_result}'  \n Received '{actual_result}'"
