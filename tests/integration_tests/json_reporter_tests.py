...
def VerifyInitialJsonReport(
    uid, chain, submission_dir, json_file, argv, some_input, original_cli=None
):
    poll_until_path_exist(json_file)
    with open(json_file) as f:
        json_content = json.load(f)
    common_json_data = {
        "chain": [t.short_name for t in app.get_app_tasks(chain)],
        "firex_id": str(uid),
    ...
    verify_inputs(
        expected_inputs={"some_input": some_input}, inputs=json_content["inputs"]
    )
...
class JsonReportsGetGenerated(FlowTestConfiguration):
    ...
    some_service_arg_dict: ClassVar[dict[str, str]] = {
        some_service_arg: some_service_arg_value
    }
    ...
    def initial_firex_options(self) -> list:
        return [
            "submit",
            "--chain",
            f"{SomePassThroughService.__name__},{VerifyInitialJsonReport.__name__}",
            "--json_file",
            self.json_file,
            f"--{self.some_service_arg}",
            self.some_service_arg_value,
        ]

    def assert_expected_firex_output(self, cmd_output, cmd_err):
        logs_path = get_log_dir_from_output(cmd_output)

        assert task_query.wait_on_keeper_complete(logs_path), (
            "Keeper database is not complete."
        )

        VerifyInitialJsonReport_task = task_query.single_task_by_name(
            logs_path, VerifyInitialJsonReport.__name__
        )
        common_json_data = VerifyInitialJsonReport_task.firex_result["common_json_data"]

        root_task = task_query.single_task_by_name(
            logs_path, get_configured_root_task().__name__
        )
        results = root_task.firex_result

        expected = {"completed": True, "results": results, **common_json_data}
        ...
        verify_inputs(
            expected_inputs=self.some_service_arg_dict, inputs=json_content["inputs"]
        )

    def assert_expected_return_code(self, ret_value):
        assert ret_value == 0, (
            f"Test expects a CLEAN run, but returned {ret_value!s}. "
