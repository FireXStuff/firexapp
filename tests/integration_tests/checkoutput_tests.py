...
def get_partial_copied_tmpfilename(logs_dir, chain_name, filename="tmp"):
    partial_filename = (
        str(os.path.basename(__file__)).split(".")[0]
        + "."
        + chain_name
        + "_"
        + filename
...
class CheckOutputNonTempFileCopiedConfig(FlowTestConfiguration):
    ...
    def initial_firex_options(self) -> list:
        return [
            "submit",
            "--chain",
            "say_something_repeatedly",
            "--timeout",
            "30",  # relying on say_something_repeatedly's soft_time_limit to terminate the task early.
            "--file",
            self.file,
        ]
...
@app.task(soft_time_limit=5)
def run_this():
    run_this_exe = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "data", "just_hang.py"
    )
