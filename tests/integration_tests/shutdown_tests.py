...
class NoBrokerLeakBase(FlowTestConfiguration):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        ...
        assert expected_error in cmd_err, (
            f"Different error expected; expected '{expected_error}', got '{cmd_err}'"
        )
...
class NoBrokerLeakOnCtrlC(NoBrokerLeakBase):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        ...
        assert isinstance(self.completed_run.inputs["pid"], int), (
            f"input not captured properly: {self.completed_run.inputs['pid']}"
        )
...
class ExplodingReport(ReportGenerator):
    ...
    def post_run_report(self, **kwargs):
        if self.primed:
            raise RuntimeError(
                "raised by post_run_report(). All your bases are belong to us"
            )
...
firexapp.submit.tracking_service._services = tuple(
    list(existing_services) + [FailingService()]
)


class NoBrokerLeakOnFailedService(NoBrokerLeakBase):
    def initial_firex_options(self) -> list:
        install_configs = os.path.join(
            test_data_dir, "tracking_services", "install-configs-failing-launcher.json"
        )
        return [
            "submit",
            "--chain",
...
class NoBrokerLeakOnBadPlugin(NoBrokerLeakBase):
    def initial_firex_options(self) -> list:
        bad_plugin = os.path.join(
            os.path.dirname(__file__), "data", "shutdown", "bad_module.py"
        )
...
class NoBrokerLeakOnCeleryFailure(NoBrokerLeakBase):
    def initial_firex_options(self) -> list:
        bad_plugin = os.path.join(
            os.path.dirname(__file__), "data", "shutdown", "fail_celery_start.py"
        )
        return ["submit", "--chain", "nop", "--plugin", bad_plugin]
...
class NoBrokerLeakOnCeleryTerminated(NoBrokerLeakBase):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        ...
        assert os.path.exists(completion_file), (
            f"RUN_COMPLETED is expected to be found in {completion_file}"
        )
...
class ShutdownDetachedFromParentProcess(NoBrokerLeakOnCeleryTerminated):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        ...
        assert shutdown_proc.name() == "firex_shutdown", (
            f"Unexpected shutdown proc name: {shutdown_proc.name()}"
        )

        child_procs = Process().children(recursive=True)
        assert shutdown_proc not in child_procs, (
            "Expected shutdown to not be a child of the current process"
        )
        shutdown_proc.wait(
            20
        )  # unfortunately can't confirm return code since it isn't a child.
