...
class InfoFindsMicroservice(FlowTestConfiguration):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        assert f"Name: a_service_to_test ({self.__module__})" in cmd_output, (
            "Test info not provided"
        )
        assert doc in cmd_output, "Documentation was not picked up"
        assert "use_this_arg" in cmd_output, (
            "positional argument was not included in output"
        )
        assert "and_maybe_this_one" in cmd_output, (
            "optional argument was not included in output"
        )
...
class InfoFindsFullNameMicroservice(FlowTestConfiguration):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        assert f"Name: a_second_service_to_test ({self.__module__})" in cmd_output, (
            "Test info not provided"
        )
        assert not cmd_err, "No errors expected"
...
class InfoCaNotFindMicroservice(FlowTestConfiguration):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        assert "Microservice does.hot.exist was not found!" in cmd_err, (
            "Error not provided"
        )
...
class ListMicroservices(FlowTestConfiguration):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        assert "The following microservices are available:" in cmd_output, (
            "List header not there"
        )
        assert "info_tests.a_service_to_test" in cmd_output, (
            "Test microservice not listed"
        )
        assert not cmd_err, "No errors expected"
...
# noinspection PyUnusedLocal
@current_app.task(base=FireXTask)
def second_to_use_arg(
    uid, use_this_arg
):  # this is the same arg name as the microservice at the top of this module
    pass
...
class ListArguments(FlowTestConfiguration):
    ...
    def assert_expected_firex_output(self, cmd_output, cmd_err):
        assert "The following arguments are used by microservices" in cmd_output, (
            "List header not there"
        )
