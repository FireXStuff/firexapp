    ...
    for string in strs:
        if not any(string.startswith(ignore) for ignore in allowed_prefixes):
            raise AssertionError(
                f"Found string '{string}' matching no expected prefix: {allowed_prefixes}"
            )
...
def assert_flame_web_ok(flame_url, path):
    ...
    assert alive_request.ok, (
        f"Expected OK response when fetching {alive_url} resource page: {path}"
    )
...
class FlameLaunchesTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        assert root_request.ok, (
            f"Expected OK response when fetching main page: {root_request.status_code}"
        )
        ...
        main_page_script_srcs = [
            s["src"] for s in main_page_bs.find_all("script") if "src" in s
        ]
        page_resource_urls = main_page_link_hrefs + main_page_script_srcs
        assert_all_match_some_prefix(
            page_resource_urls, ["/", "https://fonts.googleapis"]
        )
        ...
        assert self.chain in task_names, (
            f"Task API endpoint didn't include executed chain ({self.chain})."
        )
...
class FlameLaunchWithCentralServerTest(FlameFlowTestConfiguration):
    ...
    def initial_firex_options(self) -> list:
        return [
            ...
            self.central_server_ui_path,
        ]

    def assert_on_flame_url(self, log_dir, flame_url):
        flame_url = json_file_fn(
            get_run_metadata_file(firex_logs_dir=log_dir), lambda d: d["flame_url"]
        )
        root_request = requests.get(flame_url)
        assert root_request.ok, (
            f"Expected OK response when fetching main page: {root_request.status_code}"
        ...
        page_resource_urls = main_page_link_hrefs + main_page_script_srcs
        assert_all_match_some_prefix(
            page_resource_urls, [self.central_server_ui_url, "https://fonts.googleapis"]
        )
...
class FlameTimeoutShutdownTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        assert not psutil.pid_exists(flame_pid), (
            "Timeout should have killed flame pid, but it still exists."
        )
...
class FlameSigtermShutdownTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        flame_pid = get_flame_pid(log_dir)
        assert psutil.pid_exists(flame_pid), (
            "Flame pid should exist before being killed by SIGTERM."
        )
        kill_flame(log_dir, sig=signal.SIGTERM)
        assert not psutil.pid_exists(flame_pid), (
            "SIGTERM should have caused flame to terminate, but pid still exists."
        )
...
class FlameSigintShutdownTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        redis_pid_gone = wait_until(
            lambda: not os.path.isfile(redis_pid_file), timeout=15, sleep_for=1
        )
        assert redis_pid_gone, (
            "Expected redis pid to be gone by now due to run completion."
        )

        flame_pid = get_flame_pid(log_dir)
        assert psutil.pid_exists(flame_pid), (
            "Flame pid should exist before being killed by SIGINT."
        )
        kill_flame(log_dir, sig=signal.SIGINT)
        assert not psutil.pid_exists(flame_pid), (
            "SIGINT should have caused flame to terminate, but pid still exists."
        )
...
def wait_until_model_task_uuid_complete_runstate(log_dir, timeout=20, sleep_for=1):
    return wait_until_complete_model_tasks_predicate(
        lambda ts: all(is_task_dict_complete(t) for t in ts.values()),
        log_dir,
        timeout,
        sleep_for,
    )
...
def wait_until_complete_model_tasks_predicate(
    tasks_pred, log_dir, timeout=20, sleep_for=1
):
    def until_pred():
...
def get_task_by_name(log_dir, name):
    ...
    assert named_task_count == 1, (
        f"Expecting single task with name '{name}', but found {named_task_count}"
    )
...
class FlameRevokeNonExistantUuidTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        assert resp["response"] == FAILED_EVENT, (
...
class FlameRevokeSuccessTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        assert sleep_exists, (
            "Sleep task doesn't exist in the flame rec file, something is wrong with run."
        )
        sleep_task = get_task_by_name(log_dir, "sleep")
        assert RunStates.is_incomplete_state(sleep_task["state"]), (
            f"Expected incomplete sleep, but found {sleep_task['state']}"
        )
        ...
        assert resp["response"] == SUCCESS_EVENT, (
            f"Expected response {SUCCESS_EVENT} but received {resp['response']}"
        )
        ...
        assert RunStates.create(sleep_runstate).is_revoke(), (
            f"Expected sleep runstate to be revoked, was {sleep_runstate}"
        )
...
class FlameRevokeRootRestSuccessTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        assert root_exists, (
            "Root task doesn't exist in the flame rec file, something is wrong with run."
        )

        revoke_request = requests.get(urllib.parse.urljoin(flame_url, "/api/revoke"))
        assert revoke_request.ok, (
            f"Unexpected HTTP response to revoke: {revoke_request}"
        )

        root_revoked = wait_until_complete_model_tasks_predicate(
            lambda ts: any(
                t["name"] == "RootTask" and _is_revoked(t.get("state"))
                for t in ts.values()
            ),
            log_dir,
        )
        if not root_revoked:
            ...
            raise AssertionError(
                f"Root task not revoked after revoke request, run state: {root_task_state}"
            )
...
def WaitingParent(self, uid):
    wait_until_path_exist(
        os.path.join(uid.logs_dir, "wait_file"), timeout=10, sleep_for=0.5
    )
...
TEST_TASK_QUERY = [
    ...
    {
        "matchCriteria": {"type": "equals", "value": {"name": "WaitingParent"}},
        "selectDescendants": [{"type": "equals", "value": {"name": "GrandChild"}}],
    },
]
...
class FlameSocketIoTaskQueryTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        wait_until_web_request_ok(
            urllib.parse.urljoin(flame_url, "/alive"), timeout=10, sleep_for=1
        )

        with tempfile.NamedTemporaryFile(mode="w") as temp:
            rep_file_data = {
                "model_file_name": "fake-filename.json",
                "task_queries": TEST_TASK_QUERY,
            }
            temp.write(json.dumps(rep_file_data, sort_keys=True, indent=2))
            temp.flush()

            def does_waiting_parent_exist():
                return find_task_by_name(
                    load_task_representation(log_dir, temp.name, consider_running=True),
                    "WaitingParent",
                )

            task = wait_until(does_waiting_parent_exist, 10, 1)
            assert task, "Could not find WaitingParent"
            assert RunStates.is_incomplete_state(task.get("state")), (
                f"Task not in an incomplete stat: {task.get('state')}"
            )
        ...
        def _(update_by_uuid):
            client_tasks_by_uuid.update(
                deep_merge(client_tasks_by_uuid, update_by_uuid)
            )

        sio_client.connect(flame_url)
        sio_client.emit(
            "start-listen-task-query", data={"task_queries": TEST_TASK_QUERY}
        )
        ...
        assert find_task_by_name(client_tasks_by_uuid, "WaitingParent"), (
            "Could not find WaitingParent"
        )
...
class FlameRedisKillCleanupTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        redis_pid = int(
            Path(log_dir, Uid.debug_dirname, "redis", "redis.pid").read_text()
        )
        ...
        assert sleep_state == "task-incomplete", (
            f"Expected sleep to be incomplete, was: {sleep_state}"
        )

        root_state = get_task_by_name(log_dir, "RootTask")["state"]
        assert root_state == "task-incomplete", (
            f"Expected RootTask to be incomplete, was: {root_state}"
        )

        flame_killed = wait_until_pid_not_exist(get_flame_pid(log_dir), timeout=4)
        assert not flame_killed, (
            "Flame killed after redis shutdown -- it should survive."
        )
...
class FlameTerminateOnCompleteTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        assert flame_killed, (
            "Flame not terminated after root revoked, even though terminate_on_complete was supplied."
        )

        actual_revoke_reason = self.completed_run.revoked_details.reason
        assert actual_revoke_reason, (
            f"Expected {revoke_reason}, found {actual_revoke_reason}"
        )
...
class QuertyLiveExtraRepData(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        extra_dump_file_basename = json.loads(Path(test_query_file).read_text())[
            "model_file_name"
        ]
        dumped_file = os.path.join(
            find_flame_model_dir(log_dir), extra_dump_file_basename
        )
...
def check_live_file_monitoring(host, log_dir, flame_url):
    ...
    with tempfile.NamedTemporaryFile(mode="w", delete=False) as f:
        ...
        try:
            sio_client.connect(flame_url)
            sio_client.emit(
                "start-listen-file", data={"host": host, "filepath": f.name}
            )

            found_initial = wait_until(
                lambda: initial_content.strip() in container["content"],
                timeout=10,
                sleep_for=1,
            )
            assert found_initial, (
                "Expected initial content '{}' but was not found in: {}.".format(
            ...
            found_update = wait_until(
                lambda: update_listen_content.strip() in container["content"],
                timeout=10,
                sleep_for=1,
            )
            assert found_update, (
                "Expected update content '{}' but was not found in: {}.".format(
...
class FlameDataServiceTest(FlameFlowTestConfiguration):
    ...
    def assert_on_flame_url(self, log_dir, flame_url):
        ...
        assert service_flame_data["flame_data_result"][
            "value"
        ] == _flame_return_result_fn(RETURN)
