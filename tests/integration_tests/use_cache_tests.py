...
def verify_caching_results(
    async_result, redis_key_to_check, return_value_expected, num_invocations_expected
):
    return_value = get_results(async_result)
    assert return_value == return_value_expected, (
        f"Expected {return_value_expected!r}, got {return_value!r}"
    )
...
def TestConcat(self):
    for task in [Concat1, Concat2, Concat3, Concat4]:
        logger.debug(
            f"Test case: {task.__name__} should get cached after first invocation unless use_cache was False"
        )
        ...
        c = (
            task.s(key_to_increment=key_to_increment, a="a")
            | task.s()
            | task.s(b="")
            | task.s(b="", a="a")
...
def RunAbcConcurrently(self: FireXTask, num_runs=25):
    ...
    for group in [results_1, results_2, results_3]:
        for r in group:
            ...
            assert returned_val == expected_return, (
                f"Should have returned {expected_return}, got {returned}"
            )
        expected_return += 1
