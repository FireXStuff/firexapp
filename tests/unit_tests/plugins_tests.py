...
class DuplicateIdentificationTests(unittest.TestCase):
    def test_identify_duplicate_tasks(self):
        all_tasks = ["microservice.tasks.joey", "external.joey"]
    ...
    def test_identify_duplicate_tasks_no_dups(self):
        all_tasks = ["microservice.tasks.joey", "external.chandler"]
        results = _identify_duplicate_tasks(all_tasks, [])
        self.assertTrue(len(results) == 0)

        # We make sure a sub string is not caught
        all_tasks = ["microservice.tasks.joey", "microservice.tasks.joey_different"]
        results = _identify_duplicate_tasks(all_tasks, [])
        self.assertTrue(len(results) == 0)
    ...
    def test_identify_duplicate_tasks_prioritize(self):
        ...
        # Now we reverse the order to make sure the result is the same
        all_tasks = ["external.joey", "microservice.tasks.joey"]
        results = _identify_duplicate_tasks(all_tasks, ["microservice.tasks"])
        self.assertTrue(len(results) == 1)
        this, that = tuple(results["joey"])
        self.assertEqual(this, "external.joey")
        ...
        all_tasks = [
            "celery_queues_tests.success_test_worker",
            "RunOnMcAndWorkerTestConfig_mock.success_test_worker",
        ]
        results = _identify_duplicate_tasks(
            all_tasks, ["celery_queues_tests", "RunOnMcAndWorkerTestConfig_mock"]
        )
    ...
    def test_identify_dup_of_dup(self):
        all_tasks = ["original.joey", "first.external.joey", "second.external.joey"]
        for x in range(2):
            with self.subTest(str(x)):
                results = _identify_duplicate_tasks(
                    all_tasks, ["first.external", "second.external"]
                )
...
class ResolvePathTests(unittest.TestCase):
    ...
    def test_resolve_list(self):
        ...
        self.assertEqual(
            len(files), len(FxPluginRegistry.resolve_plugin_paths(",".join(files)))
        )
    ...
    def test_plugin_env(self):
        ...
        plugin_registry.load_plugin_modules(
            test_app, get_active_plugins(), logging.INFO
        )
        ...
        mock = os.path.join(
            os.path.dirname(__file__), "data", "plugins", "mock_plugin.py"
        )
        ...
        self.assertEqual(plugins_tests_task, test_app.tasks["mock_plugin.override_me"])
        # there is a reference to the original for use
        self.assertTrue(hasattr(plugins_tests_task, "orig"))
        self.assertEqual(
            plugins_tests_task.orig, test_app.tasks["plugins_tests.override_me_orig"]
        )
...
class PluginPrecedenceTests(unittest.TestCase):
    ...
    def test_later_plugin_file_overrides_an_earlier_plugins_internal_reference(self):
        ...
        local_plugin = os.path.join(
            os.path.dirname(__file__), "data", "plugins", "local_ref_plugin.py"
        )
        competing_plugin = os.path.join(
            os.path.dirname(__file__), "data", "plugins", "competing_helper_plugin.py"
        )
    ...
    def test_module_imported_by_a_plugin_is_overridden_by_that_plugin(self):
        ...
        child_sig = test_app.tasks[
            "nested_defs.cross_plugin_defs.task_calling_cross_helper"
        ].run()
        self.assertIsInstance(child_sig, SignatureX)
    ...
    def test_plugin_file_reimported_under_another_name_is_still_overridden(self):
        ...
        child_sig = test_app.tasks[
            "reimported_defs.reimported_plugin.task_using_reimported_helper"
        ].run()
        self.assertIsInstance(child_sig, SignatureX)
...
def test_reloading_the_very_same_plugin_file_is_not_warned_about(tmp_path, caplog):
    ...
    plugin = _write_plugin(
        str(tmp_path / "a"), "reloaded_plugin.py", "RELOADED = True\n"
    )
...
def test_is_same_plugin():
    ...
    assert _loaded("/x/p.py", file_hash=None).is_same_plugin(
        _loaded("/x/p.py", file_hash=None)
    )
...
class CDLActionTest(unittest.TestCase):
    def test_cdla(self):
        arguments, _ = plugin_support_parser.parse_known_args(
            ["--plugins", "p1.py", "--plugins", "p2.py"]
        )
        self.assertEqual(arguments.plugins, "p1.py,p2.py")

    def test_cdla_normalization(self):
        arguments, _ = plugin_support_parser.parse_known_args(
            ["--plugins", "p1.py", "--plugins", "p2.py", "--plugins", "p1.py"]
        )
