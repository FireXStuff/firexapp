...
class SubprocessRunnerTests(unittest.TestCase):
    ...
    def test_check_output(self):
        ...
        with self.subTest():
            return_val = runner(
                f"/bin/echo {TEST_TEXT}", copy_file_path=self.output_file
            )
        ...
        with self.subTest():
            with self.assertRaises(
                firex_exceptions.FireXInactivityTimeoutExpired
            ) as ee:
        ...
        with self.subTest("user injected some PYTHONPATH"):
            ...
            self.assertEqual(
                runner("/bin/echo $PYTHONPATH", shell=True, env=env).strip(), new_path
            )
        ...
        with self.subTest("user set PYTHONPATH to empty string"):
            ...
            self.assertEqual(
                runner("/bin/echo $PYTHONPATH", shell=True, env=env).strip(), ""
            )
        ...
        with self.subTest(
            "No inactivity_timeout triggered when outputting to monitored file"
        ):
            tmp_dir = "/var/tmp"
        ...
        with self.subTest(
            "Trigger inactivity_timeout when outputting to unmonitored file"
        ):
            with self.assertRaises(
                firex_exceptions.FireXInactivityTimeoutExpired
            ) as ee:
    ...
    def test_run(self):
        ...
        with self.subTest():
            return_val = runner(
                f"/bin/echo {TEST_TEXT}",
                capture_output=True,
                copy_file_path=self.output_file,
            )
            self.assertIsInstance(return_val, subprocess.CompletedProcess)
    ...
    def test_stats(self):
        ...
        if num_cpu != 1:
            ...
            self.assertTrue(
                expected - 15 < stats.cpu_percent_used < expected + 15,
        ...
        expected = 100
        self.assertTrue(
            expected - 25 < stats.cpu_percent_used < expected + 25,
            f"Expected {expected}% CPU but got {stats.cpu_percent_used}",
        )
        ...
        self.assertTrue(
            mem_expected * 0.50
            <= stats.mem_mb_used
            < mem_expected * 1.30,  # Loose check due to experience
