...
class RunSoftTimeLimitInRunJsonTests(unittest.TestCase):
    ...
    def test_a_raise_is_recorded(self):
        ...
        with _unlocked():
            recorded = FireXRunData.persist_run_soft_time_limit(self.logs_dir, 60 * 60 * 60)
    ...
    def test_fields_this_class_does_not_model_survive_a_raise(self):
        ...
        with open(_get_initial_run_json_path(self.logs_dir), 'w', encoding='utf-8') as f:
            json.dump(raw, f)
    ...
    def test_a_smaller_raise_does_not_lower_the_recorded_budget(self):
        ...
        with _unlocked():
            FireXRunData.persist_run_soft_time_limit(self.logs_dir, 60 * 60 * 60)
            recorded = FireXRunData.persist_run_soft_time_limit(self.logs_dir, 50 * 60 * 60)
    ...
    def test_the_read_modify_write_is_done_under_the_lock(self):
        ...
        with mock.patch(
            'firexapp.reporters.json_reporter._run_json_lock', spy_lock,
        ), mock.patch(
