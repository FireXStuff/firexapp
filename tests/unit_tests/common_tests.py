...
class SplitListTests(unittest.TestCase):
    def test_delimit2list(self):
        ...
        self.assertEqual(
            delimit2list("one, 'two, two',three,,,',',',four,'"),
            ["one", "two, two", "three", "four"],
        )

        self.assertEqual(
            delimit2list("'stuff', convert_list_recursive1.txt", delimiters=","),
        ...
        self.assertEqual(
            ...
            [
                "a_test_script.py --my_opt 'stuff1, stuff2'",
                "convert_list_recursive1.txt",
            ],
        )

        self.assertEqual(
            delimit2list('script.py --delimiter ","', delimiters=","),
            ['script.py --delimiter ","'],
        )
        ...
        self.assertEqual(
            delimit2list(
                'a_test_script.py --my_opt "This is a sentence"', delimiters=","
            ),
        ...
        self.assertEqual(
            delimit2list("one,two;three;four|five"),
...
class PollingTests(unittest.TestCase):
    ...
    def test_poll_till_exists(self):
        ...
        with self.assertRaises(AssertionError):
            poll_until_file_exist(
                os.path.join(os.path.dirname(__file__), "not_happening"), timeout=0.1
            )
        ...
        try:
            ...
            assert not os.path.isfile(file_name), (
                "File should not exist before the poll starts"
            )
