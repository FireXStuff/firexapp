...
class TestAdoptedDefaults:
    ...
    def test_auto_inject_args_keep_their_own_default(self):
        ...
        assert _merged(task, orig).parameters['injected'].default is inspect.Parameter.empty
...
class TestKindsAndOrderAreValid:
    ...
    def test_everything_merged_at_once(self):
        ...
        assert list(merged.parameters) == [
            'pos', 'a', 'args', 'kw_only', 'added', 'also_added', 'kwargs',
        ]
        assert _defaults(merged) == {
            'pos': 1, 'a': 2, 'args': None, 'kw_only': 4,
            'added': 3, 'also_added': 5, 'kwargs': None,
        }
        # the merged params must describe a callable python accepts.
        assert _params(inspect.Signature(list(merged.parameters.values()))) == _params(merged)
