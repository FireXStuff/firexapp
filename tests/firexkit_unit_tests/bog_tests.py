...
class BagTests(unittest.TestCase):
    def test_first_unbound_microservice(self):
        ...
        self.assertNotEqual(
            len(kwargs),
            len(bog.return_args),
            "Updating kwargs should not affect the bog",
        )
        bog.update(new_data)
        self.assertDictEqual(
            bog.return_args,
            kwargs,
            "bag of goodies should be same as kwargs after update",
        )
        ...
        self.assertDictEqual(
            bog.return_args,
            kwargs,
            "bag of goodies should be same as kwargs after update to existing key",
        )
    ...
    def test_first_with_explicit_args(self):
        ...
        self.assertDictEqual(bog.return_args, {"value": "nope"})

        bog.update({"value": "yup"})
        self.assertDictEqual(bog.return_args, {"value": "yup"})
    ...
    def test_positional_arg_is_a_dict(self):
        ...
        with self.subTest("positional dict is first arg, but no previous returns"):
            ...
            kwargs = {}
            bog = BagOfGoodies(sig, args, kwargs)
            self.assertDictEqual(bog.return_args, {"some_key": {"k1": "v1"}})
        ...
        with self.subTest("old value should be trumped by positional dict"):
            ...
            bog = _in_chain_bog(sig, args, kwargs)
            self.assertDictEqual(
                bog.return_args, {"some_key": {"k1": "v1"}, "k2": "other_value"}
            )
            a, _k = split_for_signature(bog)
            self.assertEqual(len(a), 1)
            self.assertEqual(a[0], {"k1": "v1"})
    ...
    def test_chained_unbound_microservice(self):
        ...
        self.assertDictEqual(
            bog.return_args, old_bog, "bag of goodies should be same as kwargs"
        )
        new_data = {"random": 0}
        ...
        self.assertNotEqual(
            len(old_bog),
            len(bog.return_args),
            "Updating old bog should not affect the bog",
        )
        bog.update(new_data)
        self.assertDictEqual(
            bog.return_args,
            old_bog,
            "bag of goodies should be same as old_bog after update",
        )
        mod_value = {"chain": "something"}
        ...
        self.assertDictEqual(
            bog.return_args,
            old_bog,
            "bag of goodies should be same as old_bog after update to existing key",
        )
    ...
    def test_bind_goodies(self):
        ...
        class Thing:
            ...
            def a_func(self, expected_module):
                a_self.assertEqual(
                    expected_module, "additional_for_plugin_propagation.py"
                )
