...
class EventFileProcessorTests(unittest.TestCase):
    def test_process_rec_file(self):
        ...
        recording_file = os.path.join(
            test_data_dir, "nop", "firex_internal", "flame", "flame.rec"
        )
        ...
        self.assertEqual(
            2,
            len(controller.graph.get_full_tasks_by_uuid()),
            "Expected two tasks, root and nop.",
        )
        # TODO: assert on metadata once args are emitted in celery events by firexapp.

    def test_rec_full_task(self):
        t = get_model_or_rec_full_task(
            os.path.join(test_data_dir, "nop"), "e33054ad-4a02-45ae-b9d3-f92eda6cdedd"
        )
