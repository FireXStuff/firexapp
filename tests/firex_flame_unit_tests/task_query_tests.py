...
class TaskQueryTests(unittest.TestCase):
    def test_match_all(self):
        ...
        self.assertEqual(
            result["1"], {k: v for k, v in tasks["1"].items() if k in selectPaths}
        )
        self.assertEqual(
            result["2"], {k: v for k, v in tasks["2"].items() if k in selectPaths}
        )
    ...
    def test_select_descendants(self):
        queries = _convert_json_paths_in_query(
            [
                ...
                {
                    "matchCriteria": {"type": "equals", "value": {"flame_data": 1}},
                    "selectDescendants": [
        ...
        self.assertEqual(
            result["1"],
            {
                "parent_id": None,
                "uuid": "1",
                "descendants": {"3": {"parent_id": "2", "uuid": "3"}},
            },
        )
    ...
    def test_partial_select(self):
        queries = _convert_json_paths_in_query(
            [
                ...
                {
                    "matchCriteria": {"type": "equals", "value": {"flame_data": 2}},
                    "selectDescendants": [
                        {"type": "equals", "value": {"flame_data": 3}},
                    ],
                },
                {
                    "matchCriteria": {"type": "equals", "value": {"flame_data": 3}},
                },
            ]
        )
        all_tasks = {
    ...
    def test_select_from_inconsistent_additional_children(self):
        ...
        queries = _convert_json_paths_in_query(
            [
                ...
                {
                    "matchCriteria": {"type": "equals", "value": {"uuid": "3"}},
                    "selectDescendants": [
                        {"type": "equals", "value": {"support_location": 3}},
                    ],
                },
                {
                    "matchCriteria": {"type": "equals", "value": {"flame_data": 3}},
        ...
        partial_query_result = FlameTaskGraph(all_tasks).query_partial_tasks(
            ["3"], queries
        )
        expected = {
            "3": {
    ...
    def test_select_list_from_task(self):
        task = {
            "flame_data": {
                "list": [{"ignored": 1, "field2": 1}, {"field2": 2, "field3": 3}]
            }
        }
        ...
        self.assertEqual(
            result,
            {"flame_data": {"list": [{"field2": 1}, {"field2": 2, "field3": 3}]}},
        )
