...
class TaskQueryTests(unittest.TestCase):
    def test_write_only_slim(self):
        with tempfile.TemporaryDirectory() as log_dir:
            ...
            self.assertEqual(
                controller.graph.get_slim_tasks_by_uuid(), loaded_slim_tasks
            )
    ...
    def test_write_only_tasks(self):
        with tempfile.TemporaryDirectory() as log_dir:
            ...
            controller.update_graph_and_sio_clients(
                [
                    ...
                    {"uuid": uuid2, "type": "task-succeeded"},
                    {
                        "uuid": uuid3,
                        "type": "task-blocked",
                    },  # this dumper will not write this task.
                ]
            )
            running_model_dumper._queue.join()
            ...
            self.assertEqual(
                controller.graph.get_full_task_dict(uuid2),
                load_full_task(log_dir, uuid2),
            )
            ...
            self.assertEqual(
                controller.graph.get_full_task_dict(uuid3),
                load_full_task(log_dir, uuid3),
            )
    ...
    def test_dump_extra_task_representations(self):
        with tempfile.TemporaryDirectory() as log_dir:
            ...
            expected_task_repr[uuid1]["descendants"] = {
                "2": {"name": "hello_child", "uuid": "2"}
            }
                ...
                json.dumps(
                    {
                        ...
                        "task_queries": [
                            ...
                            },
                            {
                                ...
                                "selectDescendants": [
                                    {"type": "equals", "value": {"name": "b"}},
                                    {"type": "equals", "value": {"name": "c"}},
                                ],
                            },
                            {
                                "matchCriteria": {
