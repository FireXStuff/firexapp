...
def monitor_file(sio_server, sid, host, filename):
    ...
    if host in ["127.0.0.1", "localhost", gethostname()]:
        ...
        try:
            ...
            logger.warning(
                "Exception raised while trying to spawn subprocess to monitor file: ",
                exc_info=True,
            )
            return
        ...
        try:
            ...
            if not res_out or res_out == b"":
                if not res_err or res_err == b"":
                    emit_line_data(
                        "ERROR: File access permissions prevent viewing of file.\n"
                    )
                else:
                    ...
                    if "No such file or directory" in res_err:
                        ...
                        emit_line_data(
                            "[Temporary file no longer exists - executed command has completed]\n"
                        )
                    else:
                        emit_line_data(
                            f"ERROR: Unexpected error while checking file existence and permissions: {res_err}"
                        )
                return
            else:
                ...
                if filename not in res_out:
                    emit_line_data(
                        f"ERROR: Unexpected output while checking file existence and permissions: {res_out}"
                    )
                    return
...
def create_socketio_task_api(controller: FlameAppController):
    ...
    def emit_frontend_tasks_by_uuid(sid, data=None):
        ...
        if isinstance(data, dict) and {"task_queries", "model_file_name"}.intersection(
            data.keys()
        ):
    ...
    def start_file_monitor(sid, args):
        if "host" not in args or "filepath" not in args:
            ...
            spawn(
                monitor_file,
                sio_server=controller.sio_server,
                sid=sid,
                host=args["host"],
    ...
    def start_listen_task_query(sid, args):
        if not {"task_queries", "model_file_name", "query_config"}.intersection(
            args.keys()
        ):
...
def create_revoke_api(
    ...
    def connect(sid, environ):
        requester = _data_from_environ_path(environ, authed_user_request_path, NO_USER)
        sids_to_details[sid] = {
            "requester": requester,
        }
