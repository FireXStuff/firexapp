...
if os.path.isdir(static_files_dir):
    ...
    UI_RESOURCE_DIR = str(
        file_manager.enter_context(importlib_resources.files("firex_flame_ui"))
    )
...
def _get_asset_rel_path(flame_ui_built_root_dir, asset_suffix) -> str:
    asset_glob = os.path.join(
        flame_ui_built_root_dir, "assets", f"index.*.{asset_suffix}"
    )
    found_assets = glob(asset_glob)
    assert len(found_assets) == 1, (
        f"Found {len(found_assets)} assets matching glob, require exactly 1. Glob: {asset_glob}"
    )
        ...
        if not cached_ui_index_response:
            ...
            if central_server and central_server_ui_path:
                served_app_js_url = urllib.parse.urljoin(
                    central_server, os.path.join(central_server_ui_path, rel_js_path)
                )

                served_app_css_url = urllib.parse.urljoin(
                    central_server, os.path.join(central_server_ui_path, rel_css_path)
                )
            else:
...
def create_web_app(run_metadata, serve_logs_dir):
    ...
    web_app.add_url_rule(
        ...
        create_ui_index_render_function(
    ...
    web_app.add_url_rule(
        "/img/<path:p>",
        "ui_img_artifacts",
        lambda p: redirect(REL_UI_RESOURCE_PATH + "/img/" + p),
    )

    ui_config = create_ui_config(run_metadata)
    web_app.add_url_rule(
        "/flame-ui-config.json", "ui_config", lambda: ok_json_response(ui_config)
    )
    ...
    web_app.add_url_rule(
        "/root/<uuid>", "subtree", lambda uuid: redirect("/#/root/" + uuid)
    )
