...
sys.modules["celery.events.dispatcher"] = type(
    "noop2", (object,), {"EventDispatcher": noop_class}
)
...
def _parse_args():
    ...
    parser.add_argument(
        "--port", help="Port for starting the web server on", type=int, default=0
    )
    parser.add_argument(
        "--uid", help="Unique identifier for the represented FireX run."
    )
    parser.add_argument(
        "--logs_dir", help="Logs directory.", default=None, required=True
    )
    ...
    parser.add_argument(
        "--recording",
        help="A file containing the recording of celery events.",
        default=None,
    )
    parser.add_argument(
        "--central_server",
        help="A central web server from which the UI and logs can be served.",
        default=None,
    )
    parser.add_argument(
        "--central_server_ui_path",
        help="Path part of the URL from which the central server serves"
        "the UI. Only has meaning when a central_server is supplied.",
        default=None,
    )
    parser.add_argument(
        "--logs_server",
        help="A central web server from which logs can be fetched.",
        default=None,
    )
    parser.add_argument(
        "--central_documentation_url",
        help="URL linking to main out-of-app docs.",
        default="http://www.firexapp.com/",
    )
    parser.add_argument(
        "--firex_bin_path", help="Path to a firex executable.", default=None
    )
    parser.add_argument(
        "--flame_timeout",
        help="Maximum lifetime of this service, in seconds",
        type=int,
        default=DEFAULT_FLAME_TIMEOUT,
    )
    parser.add_argument(
        "--broker_recv_ready_file",
        help="File to create immediately before capturing celery events.",
        default=None,
    )
    parser.add_argument(
...
class ShutdownHandler:
    def __init__(self):
        ...
        signal.signal(
            signal.SIGHUP,
            lambda _, __: logger.warning(
                "Ignoring SIGHUP signal -- will timeout with broker."
            ),
        )
...
def main():
    ...
    t = (
        ...
        else Timer(args.flame_timeout, shutdown_handler.timeout_handler)
    )
    try:
        t.start()
        ...
        print(
            f"Flame server running on: {get_flame_url_from_port(web_server.server_port)}"
        )
