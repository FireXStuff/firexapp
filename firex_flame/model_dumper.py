...
def get_all_tasks_dir(firex_logs_dir=None, root_model_dir=None):
    return os.path.join(
        _get_base_model_dir(firex_logs_dir, root_model_dir), "full-tasks"
    )


def get_run_metadata_file(firex_logs_dir=None, root_model_dir=None):
    return os.path.join(
        _get_base_model_dir(firex_logs_dir, root_model_dir), "run-metadata.json"
    )
...
def wait_and_get_flame_url(
    firex_logs_dir: str, timeout=15, sleep_for=0.5
) -> str | None:
    return wait_until(
        _get_flame_url,
        timeout=timeout,
...
def get_tasks_slim_file(firex_logs_dir=None, root_model_dir=None):
    return os.path.join(
        _get_base_model_dir(firex_logs_dir, root_model_dir), "slim-tasks.json"
    )


def get_model_complete_file(firex_logs_dir=None, root_model_dir=None):
    return os.path.join(
        _get_base_model_dir(firex_logs_dir, root_model_dir), "model-complete"
    )
...
def load_task_representation(
    firex_logs_dir, representation_file, consider_running=False
):
    with open(representation_file) as fp:
        representation_data = json.load(fp)

    task_rep_file = os.path.join(
        _get_base_model_dir(firex_logs_dir=firex_logs_dir),
    ...
    if os.path.isfile(task_rep_file):
        ...
        if os.path.isfile(run_metadata_file):
            ...
            return socketio_client_task_queries_request(
                run_metadata["flame_url"], representation_data["task_queries"]
            )
...
def get_model_full_tasks_by_names(log_dir, task_names):
    ...
    tasks_by_uuid = get_full_tasks_by_slim_pred(
        log_dir, lambda st: st.get("name", None) in task_names
    )
...
def get_model_slim_tasks_by_names(log_dir, task_names):
    ...
    tasks_by_uuid = get_slim_tasks_by_slim_pred(
        log_dir, lambda st: st.get("name") in task_names
    )
    return index_tasks_by_names(tasks_by_uuid.values(), task_names)
