...
def _get_event_lag(event_timestamp, celery_utcoffset):
    ...
    try:
        ...
        event_datetime = datetime.fromtimestamp(
            event_timestamp, timezone(timedelta(hours=real_tz_offset_hr))
        )
...
class BrokerEventConsumerThread(threading.Thread):
        ...
        try:
            ...
            if self.terminate_on_complete and not self.stopped_externally:
                self.shutdown_handler.shutdown(
                    "Terminating on completion, as requested by input args."
                )
        ...
        while not self.flame_controller.is_root_complete():
            try:
                ...
                if self.flame_controller.is_root_complete():
                    logger.info(
                        "Root task complete; stopping broker receiver (not entire server)."
                    )
                    return
                if self.shutdown_handler.shutdown_received:
                    self.stopped_externally = True
                    logger.info(
                        "Shutdown handler received shutdown request; stopping broker receiver."
                    )
                    return
                logger.error(traceback.format_exc())
                if try_interval > self.max_try_interval:
                    logger.warning(
                        "Maximum broker retry attempts exceeded, stopping receiver (not entire server)."
    ...
    def _on_celery_event(self, event):
        ...
        if self.flame_controller.is_all_tasks_complete() and self.celery_event_receiver:
            logger.info(
