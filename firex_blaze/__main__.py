...
def _parse_blaze_args():
    ...
    parser.add_argument(
        "--logs_dir",
        help="Logs directory for the run to keep task data for.",
        required=True,
    )
    parser.add_argument(
        "--uid", help="FireX UID for the run to keep task data for.", required=True
    )
    parser.add_argument(
        "--firex_requester",
        help="Requester username, if different than running user",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--logs_url",
        help="Webserver used from which logs can be accessed.",
        default=None,
    )

    parser.add_argument(
        "--kafka_topic", help="Topic use for the Kafka bus", required=True
    )
    parser.add_argument(
        "--bootstrap_servers",
        help="Comma seperated list of Kafka bootrap servers.",
        required=True,
    )
    parser.add_argument(
        "--security_protocol",
    ...
    parser.add_argument(
        "--sasl_mechanism", help="SASL mechanism (e.g., OAUTHBEARER).", default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--sasl_oauthbearer_client_id", help="OAuth client ID.", default=None
    )
    parser.add_argument(
        "--sasl_oauthbearer_client_secret", help="OAuth client secret.", default=None
    )
    parser.add_argument(
...
def init_blaze():
    ...
    run_metadata = FireXRunMetadata(
        args.uid, args.logs_dir, None, None, args.firex_requester
    )
