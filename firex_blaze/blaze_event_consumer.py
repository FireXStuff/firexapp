...
BLAZE_SEND_EVENT_TYPES = tuple(
    list(TASK_EVENT_TO_STATE.keys())
    + ["task-completed", "task-results", "task-instrumentation", "task-args"]
)
...
def format_kafka_message(
    ...
    return {
        ...
        "EVENTS": [{"DATA": event_data, "UUID": uuid}],
    }


def send_kafka_mssg(
    kafka_producer: Producer,
    kafka_mssg: dict[str, Any],
    kafka_topic: str,
    firex_id: str,
...
class KafkaSenderThread(BrokerEventConsumerThread):
        ...
        while True:
            try:
                ...
                if _retries < config.max_kafka_connection_retries:
                    ...
                    logger.warning(
                        f"Retrying connecting to bootstrap servers "
                        f"[retry {_retries}/{config.max_kafka_connection_retries}]"
                    )
                    time.sleep(min(2**_retries, 30))  # Exponential backoff
                else:
                    logger.error(
                        f"Failed to connect to Kafka after {config.max_kafka_connection_retries} retries"
                    )
                    raise
    ...
    def _send_celery_event_to_kafka(
        self, celery_event: dict[str, Any]
    ) -> list[dict[str, Any]]:
...
class BlazeKafkaSenderThread(KafkaSenderThread):
    ...
    def _get_kafka_event(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        if uuid in self.uuid_to_task_name_mapping:
            ...
            raise NoNameForEvent(
                f"No task name found for {event}; can not send the event"
            )
