...
class FireXBlazeLauncher(TrackingService):
    ...
    def extra_cli_arguments(self, arg_parser):
        ...
        arg_parser.add_argument(
            "--disable_blaze",
            "-disable_blaze",
            help="Disable blaze data collection",
            default=None,
            const=True,
            nargs="?",
        ...
        arg_parser.add_argument(
            "--blaze_kafka_topic", help="Topic used for Blaze's Kafka bus", default=None
        )

        arg_parser.add_argument(
            "--blaze_bootstrap_servers",
            help="Comma separated list of Kafka bootrap servers.",
            default=None,
        )

        arg_parser.add_argument(
            "--blaze_security_protocol",
        ...
        arg_parser.add_argument(
            "--blaze_sasl_mechanism", help="SASL mechanism to use (e.g., OAUTHBEARER)."
        )
        arg_parser.add_argument(
            "--blaze_sasl_oauthbearer_method", help="OAuth bearer method (e.g., oidc)."
        )
        arg_parser.add_argument(
            "--blaze_sasl_oauthbearer_client_id", help="OAuth client ID."
        )
        arg_parser.add_argument(
            "--blaze_sasl_oauthbearer_client_secret", help="OAuth client secret."
        )
        arg_parser.add_argument(
            "--blaze_sasl_oauthbearer_token_endpoint_url",
            help="OAuth token endpoint URL.",
        )
        arg_parser.add_argument(
            "--blaze_ssl_ca_location",
    ...
    def _create_blaze_command(cls, uid, args, broker_recv_ready_file):
        cmd = [
            ...
            uid.logs_url,
            "--kafka_topic",
            args.blaze_kafka_topic,
            "--bootstrap_servers",
            args.blaze_bootstrap_servers,
            "--instance_name",
            cls.instance_name,
            "--security_protocol",
            args.blaze_security_protocol,
        ]
        ...
        if (
            hasattr(args, "blaze_sasl_oauthbearer_method")
            and args.blaze_sasl_oauthbearer_method
        ):
            cmd += ["--sasl_oauthbearer_method", args.blaze_sasl_oauthbearer_method]
        if (
            hasattr(args, "blaze_sasl_oauthbearer_client_id")
            and args.blaze_sasl_oauthbearer_client_id
        ):
            cmd += [
                "--sasl_oauthbearer_client_id",
                args.blaze_sasl_oauthbearer_client_id,
            ]
        if (
            hasattr(args, "blaze_sasl_oauthbearer_client_secret")
            and args.blaze_sasl_oauthbearer_client_secret
        ):
            cmd += [
                "--sasl_oauthbearer_client_secret",
                args.blaze_sasl_oauthbearer_client_secret,
            ]
        if (
            hasattr(args, "blaze_sasl_oauthbearer_token_endpoint_url")
            and args.blaze_sasl_oauthbearer_token_endpoint_url
        ):
            cmd += [
                "--sasl_oauthbearer_token_endpoint_url",
                args.blaze_sasl_oauthbearer_token_endpoint_url,
            ]
    ...
    def start(
        ...
        sufficient_args = (
            uid.logs_url and args.blaze_kafka_topic and args.blaze_bootstrap_servers
        )
        if args.disable_blaze or not sufficient_args:
            ...
            if not sufficient_args:
                logger.warning(
                    "Blaze did not receive sufficient arguments; will not launch subprocess."
                )
        ...
        self.broker_recv_ready_file = os.path.join(
            blaze_debug_dir, "blaze_celery_recvr_ready"
        )
        ...
        try:
            ...
            logger.error(
                "Failed to start FireXBlaze -- task data will not be put on Kafka bus."
            )
    ...
    def get_pkg_version_info() -> PkgVersionInfo:
        ...
        return PkgVersionInfo(pkg="firex-blaze", version=firex_blaze.__version__)
