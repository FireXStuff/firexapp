...
def init_keeper():
    ...
    run_metadata = FireXRunMetadata(
        args.uid, args.logs_dir, args.chain, None, firex_requester=None
    )
...
def main():
    ...
    try:
        TaskDatabaseAggregatorThread(
            celery_app, run_metadata, receiver_ready_file=receiver_ready_file
        ).run()
    except Exception:
